...
struct LogData
{
   static Container::MultipleProducerLockFreeQueue<std::string, 128u> ms_LogQueue;
};
//---------------------------------------------------------------------------------//
...
struct LogEntry
{
   LogEntry(std::string p_Log, Severity p_LogSeverity, const char* const p_FileName, const char* const p_FunctionName, long p_Line)
...
struct ConsoleModule
{
   ...
   {
      std::string string = simpleSprintf("%s %s", t_TypeInfo::getPrefix(), p_LogEntry.log.c_str());
